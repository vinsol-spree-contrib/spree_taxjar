  ...
  class Taxjar
    ...
    private
      ...
      def tax_params
        {
          ...
          shipping: @order.shipment_total + @order.shipment_adjustments.where.not(source_type: "Spree::TaxRate").pluck(:amount).sum,
      ...
      def shipment_tax_params
        address_params.merge({
          amount: 0,
          shipping: @shipment.cost + @shipment.adjustments.where.not(source_type: "Spree::TaxRate").pluck(:amount).sum.to_f
        })
      end
